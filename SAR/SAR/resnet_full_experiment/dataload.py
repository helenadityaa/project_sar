...
class OpenSARShipDataset(Dataset):
    ...
    def filter_data(self):
        """Filter dataset for 4 target ship classes: Cargo, Fishing, Tanker, Other Type"""
        ...
        for _, row in self.ais_data.iterrows():
            ship_type = str(row.get(ship_type_col, ""))

            # --- Map to 4 classes ---
            # 0: Cargo, 1: Fishing, 2: Tanker, 3: Other Type
            label = None
            if "Cargo" in ship_type:
                label = 0
            elif "Fishing" in ship_type:
                label = 1
            elif "Tanker" in ship_type:
                label = 2
            else:
                # All other types (Tug, Dredging, etc.) go to 'Other Type'
                label = 3
