...
def create_balanced_dataset(dataset, target=-1, is_augmented=False):
    ...
    for class_id, indices in class_indices.items():
        ...
        if current_count < target:
            ...
            # Aggressive augmentation: apply a random combination of 1-3 transforms per synthetic sample.
            ...
            while n_augment > 0:
                ...
                # Pick a random number of effects (1 to 3) to apply in sequence
                num_effects = random.randint(1, 3)
                selected_augs = random.sample(aug_names, num_effects)
                
                # Apply the combination of effects
                augmented_img = augmentation.apply_augmentations(original_img, selected_augs)
                
                stem = Path(original_sample["img_path"]).stem
                suffix = Path(original_sample["img_path"]).suffix
                # Create a filename that reflects the multiple augmentations
                aug_suffix = "_".join(selected_augs)
                filename = f"augment/{aug_suffix}_{ptr}_{stem}{suffix}"
...
# Lokasi script ini (di dalam folder eksperimen)
base_dir = Path(__file__).resolve().parent
# Lokasi project utama (satu tingkat di atas folder eksperimen)
project_root = base_dir.parent

# Mencari folder PATCH_CAL di folder project utama
patch_cal_dir = project_root / "PATCH_CAL"
if not patch_cal_dir.exists():
    # Coba alternatif lokasi lain jika tidak ketemu
    alt_patch_cal_dir = project_root / "dataset" / "PATCH_CAL"
    if alt_patch_cal_dir.exists():
        patch_cal_dir = alt_patch_cal_dir
    else:
        raise FileNotFoundError(
            f"Folder PATCH_CAL tidak ditemukan di: {patch_cal_dir} atau {alt_patch_cal_dir}"
        )
...
for file_path in patch_cal_dir.iterdir():
    ...
    # Increase resolution to 128x128 for better detail
    img_resized = resize(img, (128, 128), order=3, mode='reflect', preserve_range=True)
    tiff.imwrite("resized_new/" + file_path.name, img_resized.astype(np.float32))

print("\nLoading datasets...")
# Menggunakan metadata.csv dari folder project utama
dataset = OpenSARShipDataset(root_dir=str(project_root), image_dir=base_dir / "resized_new")
